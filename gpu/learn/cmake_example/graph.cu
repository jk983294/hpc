...
void cudaGraphsManual(float *inputVec_h, float *inputVec_d, double *outputVec_d, double *result_d, size_t inputSize,
                      size_t numOfBlocks) {
    ...
    checkCudaErrors(cudaGraphCreate(&graph, 0));  // manual create starts
    ...
    checkCudaErrors(cudaGraphAddKernelNode(&kernelNode, graph, nodeDependencies.data(), nodeDependencies.size(),
                                           &kernelNodeParams));
...
void cudaGraphsUsingStreamCapture(float *inputVec_h, float *inputVec_d, double *outputVec_d, double *result_d,
    ...
    checkCudaErrors(cudaStreamBeginCapture(stream1));  // graph definition starts
    ...
    checkCudaErrors(cudaStreamEndCapture(stream1, &graph));  // graph definition ends
